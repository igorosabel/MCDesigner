...
export class EditDesignComponent implements OnInit {
  ...
  position: Point = { ...this.initialPosition };
  ...
  line: Line = {
    start: { x: -1, y: -1 } as Point,
    end: { x: -1, y: -1 } as Point,
  };
  ...
  toolsDragEnd(event: CdkDragEnd): void {
              ...
              if (result.status == "ok") {
                this.dialog
                  .alert({
                    title: "Success",
                    content:
                      'New level "' + newLevel.name + '" has been added.',
                    ok: "Continue",
                  })
                  .subscribe((result) => {});
            ...
            this.as.renameLevel(levelData).subscribe((result) => {
              if (result.status == "ok") {
                this.dialog
                  ...
                  .subscribe((result) => {});
                const ind = this.design.levels.findIndex(
                  (x) => x.id == level.id
                );
            ...
            if (result.status == "ok") {
              ...
              this.dialog
                .alert({
                  title: "Success",
                  content:
                    'New level "' + result.level.name + '" has been added.',
                  ok: "Continue",
                })
                .subscribe((result) => {});
              this.design.levels.push(result.level);
  ...
  fillAddCell(p: Point): void {
    ...
    for (let newP of surrounding) {
      if (
        ...
        const ind = this.fillToBePainted.findIndex(
          (e) => e.x == newP.x && e.y == newP.y
        );
        if (ind == -1) {
